		...
		for(int i = 0; i < Num; i++)
		{
			...
			if(Item.m_Type == NETOBJTYPE_CLIENTINFO)
			{
				...
				if(ClientId < MAX_CLIENTS)
				{
					...
					// pClient->m_pSkinInfo.m_Size = 64;
					if (g_Config.m_ClDuckFilter)
						// pClient->m_pSkinInfo.Apply(m_Skins.Find("Scrubby Duck"));
						str_copy(pClient->m_aSkinName, "Scrubby Duck");
					// else
					// 	pClient->m_pSkinInfo.Apply(m_Skins.Find(pClient->m_aSkinName));
					// pClient->m_pSkinInfo.ApplyColors(pClient->m_UseCustomColor, pClient->m_ColorBody, pClient->m_ColorFeet);
