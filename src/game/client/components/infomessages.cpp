...
void CInfoMessages::OnInit()
{
	...
	for(int i = 0; i < static_cast<int>(EDamageType::COUNT); ++i)
	{
		...
		if(SpriteIndex >= 0)
		{
			Graphics()->GetSpriteScale(&g_pData->m_aSprites[SpriteIndex], ScaleX, ScaleY);
		}
...
void CInfoMessages::OnInfcKillMessage(const CNetMsg_Inf_KillMsg *pMsg)
{
	...
	if(AssistantId >= 0 && AssistantId < MAX_CLIENTS && GameClient()->m_aClients[AssistantId].m_Active)
	{
		Kill.m_AssistantId = AssistantId;
		str_copy(Kill.m_aAssistantName, GameClient()->m_aClients[Kill.m_AssistantId].m_aName);
		Kill.m_pAssistantManagedTeeRenderInfo = GameClient()->CreateManagedTeeRenderInfo(GameClient()->m_aClients[Kill.m_AssistantId]);
	}
