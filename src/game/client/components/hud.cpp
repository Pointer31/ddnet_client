...
void CHud::RenderObjectOwnerIcons(int ClientId)
{
	...
	int Icons = GameClient()->m_aClients[ClientId].m_OwnerIcons;
	if(!Icons)
		return;
	...
	float aPoints[4];
	Graphics()->MapScreenToWorld(GameClient()->m_Camera.m_Center.x, GameClient()->m_Camera.m_Center.y, 100.0f, 100.0f, 100.0f, 0, 0, Graphics()->ScreenAspect(), 1.0f, aPoints);
	Graphics()->MapScreen(aPoints[0], aPoints[1], aPoints[2], aPoints[3]);
	...
	{
		const vec2 &Pos = GameClient()->m_aClients[ClientId].m_aOwnerIconPositions[i];
...
void CHud::RenderClassExtraHud(int ClientId)
{
	...
	if(PlayerClass == PLAYERCLASS_HERO)
	{
		...
		int FlagTick = GameClient()->m_InfClassHeroGiftTick;
		char aBuffer[32];
...
void CHud::OnRender()
{
...
#endif
	{
		if(GameClient()->m_Snap.m_pLocalCharacter && !GameClient()->m_Snap.m_SpecInfo.m_Active && !(GameClient()->m_Snap.m_pGameInfoObj->m_GameStateFlags & GAMESTATEFLAG_GAMEOVER))
		{
			...
			{
				RenderClassExtraHud(GameClient()->m_aLocalIds[g_Config.m_ClDummy]);
			}
		}
		else if(GameClient()->m_Snap.m_SpecInfo.m_Active)
		{
