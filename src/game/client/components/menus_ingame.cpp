...
void CMenus::RenderInGameNetwork(CUIRect MainView)
{
	...
	else
	{
		...
		for(const CCommunity *pCommunity : ServerBrowser()->FavoriteCommunities())
		{
			...
			if(DoButton_MenuTab(&s_aFavoriteCommunityButtons[FavoriteCommunityIndex], pName, g_Config.m_UiPage == Page, &Button, IGraphics::CORNER_NONE, nullptr, nullptr, nullptr, nullptr, 10.0f, m_CommunityIcons.Find(pCommunity->Id())))
			{
				NewPage = Page;
			}
