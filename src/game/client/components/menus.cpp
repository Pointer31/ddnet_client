...
void CMenus::RenderPopupFullscreen(CUIRect Screen)
{
	...
	else if(m_Popup == POPUP_START_SERVER)
	{
		...
		if (str_comp(aBuf, "") != 0) {
			...
			// const char* arguments[3] = {bBuf, NULL};
			...
			if(str_find(aBuf, "/") == 0 || fs_is_file(aBuf))
			{
				...
				char aPass[64];
				str_format(aPass, sizeof(aPass), "sv_rcon_password %s", aRandomPass, aRandomPass);
				const char *apArguments[] = {aPass};
				// RunServer(apArguments, std::size(apArguments));
				// str_copy(g_Config.m_ClLocalServerRconpwd, aPass);
				GameClient()->m_LocalServer.m_Process = shell_execute(aBuf, EShellExecuteWindowState::BACKGROUND, apArguments, std::size(apArguments));
				m_ForceRefreshLanPage = true;
