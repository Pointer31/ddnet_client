...
void CItems::RenderPickupCustom(const CNetObj_PickupCustom *pPrev, const CNetObj_PickupCustom *pCurrent, bool IsPredicted, int Flags)
{
	float IntraTick = IsPredicted ? Client()->PredIntraGameTick(g_Config.m_ClDummy) : Client()->IntraGameTick(g_Config.m_ClDummy);
	vec2 Pos = mix(vec2(pPrev->m_X, pPrev->m_Y), vec2(pCurrent->m_X, pCurrent->m_Y), IntraTick);
...
void CItems::RenderInfclassObject(const CNetObj_InfClassObject *pCurrent, bool IsPredicted)
{
	...
	if((CameraCID >= 0) && (pCurrent->m_Owner == CameraCID))
	{
		...
		CGameClient::CClientData &ClientData = GameClient()->m_aClients[CameraCID];
		std::size_t &Icons = ClientData.m_OwnerIcons;
		if(Icons == std::size(ClientData.m_aOwnerIconPositions))
			return;

		ClientData.m_aOwnerIconPositions[Icons] = IndicatorPos;
		++Icons;
	}
#else
	if(pCurrent->m_Owner == LocalCID)
	{
		CTeeRenderInfo RenderInfo = GameClient()->m_aClients[LocalCID].m_RenderInfo;
...
void CItems::RenderSoldierBomb(const CNetObj_InfClassObject *pCurrent, bool IsPredicted)
{
	...
	if(GameClient()->m_GameSkin.m_aSpriteWeaponProjectiles[CurWeapon].IsValid())
	{
		...
		int NumBombs = GameClient()->m_InfClassSoldierBombs;
		...
		for(int i = 0; i < ActiveBombs; ++i)
		{
			...
			GameClient()->m_Effects.SmokeTrail(Pos, vec2(0, 0), Alpha, 0.0f);
...
void CItems::OnRender()
{
	...
	for(CGameClient::CClientData &Client : GameClient()->m_aClients)
	{
		Client.m_OwnerIcons = 0;
	}
	...
	for(const CSnapEntities &Ent : GameClient()->SnapEntities())
	{
		...
		{			
			const void *pPrev = Client()->SnapFindItem(IClient::SNAP_PREV, Item.m_Type, Item.m_Id);
			if(pPrev)
				RenderPickupCustom((const CNetObj_PickupCustom *)pPrev, (const CNetObj_PickupCustom *)pData, false, 0);
		}
