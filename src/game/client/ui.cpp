...
const char *CUi::ReplaceHardcodedGameName(char *pDest, const char *pSrc, int DestSize)
{
	if(pDest == pSrc)
	{
		...
		if(sizeof(s_aBuf) < static_cast<size_t>(DestSize))
		{
			dbg_msg("GUI", "Unable to replace hardcoded game name: message too long: %d", DestSize);
			return pSrc;
		}
		str_copy(s_aBuf, pSrc, sizeof(s_aBuf));
		pSrc = s_aBuf;
	}
...
{
	static char s_aBuffer[256];
	ReplaceHardcodedGameName(s_aBuffer, pSrc, sizeof(s_aBuffer));
...
{
	m_Screen.h = g_Config.m_ClUIWidth;
